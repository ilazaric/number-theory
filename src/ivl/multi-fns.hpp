...
namespace ivl::nt {
  ...
  static_assert(test_equality<Factorization<std::uint32_t>>(sigma_compiletime, sigma_dirichlet, 100));

  constexpr auto id_compiletime = compiletime::multiplicative_completion<pow>;
  static_assert(test_equality<Factorization<std::uint32_t>>(id_compiletime, id, 100));
